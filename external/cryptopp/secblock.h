...
public:
	...
	// size_t是为了方便系统之间的移植而定义的，在32位系统上定义为 unsigned int，在64位系统上定义为 unsigned long，该类型保证能容纳实现所建立的最大对象的字节大小。
...
///  on the Crypto++ wiki.
/// \since Crypto++ 2.0
// 这里用到了类模版，使代码可以复用，class A = AllocatorWithCleanup<T>是缺省值，类比函数理解
template <class T, class A = AllocatorWithCleanup<T> >
// Crypto库使用SecBlock类提供安全存储，当块被销毁时，该存储将被置零或擦除。
// 该类应该用于保存键、初始化向量和其他安全参数。
class SecBlock
{
public:
